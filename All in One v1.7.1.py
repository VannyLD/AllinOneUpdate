...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.main.radioButton_fbv2.toggled.connect(self.pin_check)
  ...
  #      self.create_folder_instagram.connect(self.move_files_to_folders_instagram)
    ...
    def pin_check(self):
        ...
        if self.main.radioButton_fbv2.isChecked():
            ...
            self.main.progressBar_google.setVisible(False)
            self.main.pushButton_stop_pic.clicked.disconnect()
            self.main.pushButton_stop_pic.clicked.connect(self.stop_download_fbv2)
            self.main.pushButton_start_pic.clicked.disconnect()
            self.main.pushButton_start_pic.clicked.connect(self.start_fbv2)
            self.main.pushButton_set_path_pic.clicked.disconnect()
            self.main.pushButton_set_path_pic.clicked.connect(self.browse_folder_fbv2)
            self.main.pushButton_past_pic.clicked.disconnect()
            self.main.pushButton_past_pic.clicked.connect(self.past_fbv2)
            self.main.lineEdit_keyword.setPlaceholderText(f"សូមបញ្ចូល Facebook Profile URL")
            MainWindow.clear_console()
    ...
    def extract_version(self, file_name):
        """ Extract version numbers like v1.7.3 from a filename """
        match = re.search(r'v(\d+\.\d+\.\d+)', file_name)  # Match 'v1.7.3'
        return match.group(1) if match else "0.0.0"

    def version_tuple(self, version):
        """ Convert version string '1.7.3' into a tuple (1,7,3) for proper comparison """
        return tuple(map(int, version.split(".")))

    def get_latest_version(self, file_name):
        """ Get the latest version of the file from GitHub """
        try:
            response = requests.get(github_repo)
            if response.status_code == 200:
                files = response.json()
                file_names = [f["name"] for f in files]
             #    print("GitHub Files:", file_names)  # DEBUGGING LINE

                # Match only files that start with "All in One v" and end with .py or .exe
                matching_files = [f for f in file_names if re.match(r'All in One v\d+\.\d+\.\d+\.(py|exe)', f)]
             #    print("Matching Files:", matching_files)  # DEBUGGING LINE

                if matching_files:
                    latest_file = max(matching_files, key=lambda f: self.version_tuple(self.extract_version(f)))
                    return latest_file
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error checking latest version: {e}")
        return None

    def download_latest_file(self, latest_file):
        """ Download the latest file and restart """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                QMessageBox.information(self, "Update", f"Update successful! New version saved as {latest_file}. Restarting...")
                os.startfile(file_path)
                sys.exit()
            else:
                QMessageBox.critical(self, "Error", "Failed to download the new version.")
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error downloading update: {e}")

    def check_for_updates(self):
        """ Check if an update is available and prompt the user """
        file_name = os.path.basename(sys.argv[0])  # Get the current script/executable name
        current_version = self.extract_version(file_name)
        latest_version_file = self.get_latest_version(file_name)

        if latest_version_file:
            latest_version = self.extract_version(latest_version_file)
            ...
            if self.version_tuple(latest_version) > self.version_tuple(current_version):
                ...
                if reply == QMessageBox.Yes:
                    self.download_latest_file(latest_version_file)
            else:
                QMessageBox.information(self, "Up to Date", "Your application is already up to date!")
        else:
            QMessageBox.information(self, "Error", "Could not retrieve the latest version.")
    ...
    def browse_folder_fbv2(self):
        folder = QFileDialog.getExistingDirectory(self, "រក្សាទុកទីតាំងឯកសា")
        if folder:
            self.main.lineEdit_path_pic.setText(folder)

    def start_fbv2(self):
        urls = self.main.lineEdit_keyword.text().strip().splitlines()
        save_folder = self.main.lineEdit_path_pic.text().strip()
        if len(urls) > 1:
            choice = QMessageBox.question(self, "ចង់ទាញយកបានច្រើន", 
                                        "ប្រសិនបើចង់ទាញយកបានច្រើន.\n\nអ្នកអាច Customize បានដោយបង់ប្រាក់បន្ថែម សូមទាក់ទងទៅ Admin ",
                                        QMessageBox.Yes | QMessageBox.No)
            if choice == QMessageBox.Yes:
                webbrowser.open("https://t.me/devmaxtools")
                return
            urls = urls[:1]
        if not urls or not save_folder:
            QMessageBox.critical(self, "ព្រមាន", "សូមបញ្ចូលទីតាំង Folder ដើម្បីរក្សាទុក")
            return
        ...
        self.main.pushButton_start_pic.setDisabled(True)
        self.main.pushButton_stop_pic.setEnabled(True)
        threading.Thread(target=self.process_urls_fbv2, args=(save_folder,), daemon=True).start()

    def process_urls_fbv2(self, save_folder):
        while self.is_downloading and self.current_url_index < len(self.url_list):
            current_url = self.url_list[self.current_url_index]
            self.main.textEdit_input_links_pic.append(f"កំពុងទាញយក: {current_url}")
            self.download_media_fbv2(current_url, save_folder)
            self.current_url_index += 1
        self.is_downloading = False
        self.main.pushButton_start_pic.setEnabled(True)
        self.main.pushButton_stop_pic.setDisabled(True)
        self.main.textEdit_input_links_pic.append("ការទាញយកជោគជ័យ")

    def download_media_fbv2(self, url, save_folder):
        exe_path = os.path.join(os.getcwd(), "File", "gallery-dl.exe")
        if not os.path.isfile(exe_path):
            self.main.textEdit_input_links_pic.append("ព្រមាន", "បាត់ឯកសារសំខាន់សម្រាប់ទាញយក")
            return
        command = [exe_path, "-d", save_folder, url]
        try:
            ...
            for line in process.stdout:
                if not self.is_downloading:
                    process.terminate()
                    break
                self.main.textEdit_input_links_pic.append(line.strip())
            process.communicate()
        except Exception as e:
            self.main.textEdit_input_links_pic.append(f"Error during download: {e}")
        finally:
            self.main.textEdit_input_links_pic.append(f"ការទាញយកជោគជ័យ: {url}")

    def stop_download_fbv2(self):
        self.is_downloading = False
        self.main.pushButton_start_pic.setEnabled(True)
        self.main.pushButton_stop_pic.setDisabled(True)
        self.main.textEdit_input_links_pic.append("ការទាញយកត្រូវបានបញ្ឃប់ដោយអ្នក")
    ...
    def past_fbv2(self):
        ...
        try:
            ...
            self.main.lineEdit_keyword.setText("\n".join(links)) 
            self.main.label_count_links_pic.setText(f"ចំនួនទាញយក៖ {CountLinkDown} ប្រូហ្វាល")
            self.main.label_count_link_success_pic.setText(f"ចំនួនទាញយក៖ {CountLinkDown} ប្រូហ្វាល")
